...
async def lifespan(app: FastAPI):
    if WEIGHTS_PATH.exists():
        ...
        app_state["error"] = f"Weights not found at {WEIGHTS_PATH}."
        log.warning(app_state["error"])
    yield


app = FastAPI(title="EMNIST Recognition API", version="1.0.0", lifespan=lifespan)
...
def _require_model() -> EMNISTNet:
    if app_state["model"] is None:
        raise HTTPException(status_code=503, detail=app_state.get("error") or "Model not loaded.")
...
@app.get("/api/classes")
def get_classes():
    return {"num_classes": NUM_CLASSES, "labels": CLASS_LABELS}
...
async def predict_upload(file: UploadFile = File(...)):
    model = _require_model()
    ALLOWED_TYPES = {"image/png", "image/jpeg", "image/jpg", "image/bmp", "image/webp", "image/gif"}
